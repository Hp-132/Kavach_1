...
function initForms() {
    ...
    document.getElementById('contact-form').onsubmit = async (e) => {
        ...
        if (res.ok) {
            ...
            addSMSBubble(`SYSTEM: Trusted contact '${contactData.name}' has been added and synced to your KAVACH device.`, 'received');
        }
    };
...
async function triggerSOS() {
    ...
    try {
        ...
        if (response.ok) {
            showAlert('🚨 EMERGENCY TRIGGERED', 'Priority SOS messages sent to all contacts.');
...
async function loadContacts() {
    ...
    data.forEach(c => {
        ...
        div.innerHTML = `
            ...
            <button onclick="deleteContact(${c.id})" style="background: none; border: none; color: #ff6b6b; cursor: pointer;"><i class="fas fa-trash"></i></button>
        `;
